...
// everything here contains virtual addresses, the mapping to physical addresses
// are in the pkt_buf
struct mempool {
  void *base_addr;
  uint32_t buf_size;
  ...
  // the stack contains the entry id, i.e., base_addr + entry_id * buf_size is
  // the address of the buf
  uint32_t free_stack[];
};
