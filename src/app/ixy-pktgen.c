...
static struct mempool *init_mempool() {
  ...
  // pre-fill all our packet buffers with some templates that can be modified
  // later we have to do it like this because sending is async in the hardware;
  ...
  // return them all to the mempool, all future allocations will return bufs
  // with the data set above
  for (int buf_id = 0; buf_id < NUM_BUFS; buf_id++) {
...
int main(int argc, char *argv[]) {
  ...
  // tx loop
  while (true) {
    // we cannot immediately recycle packets, we need to allocate new packets
    // every time the old packets might still be used by the NIC: tx is async
