...
static void init_link(struct ixy_device *dev) {
  // should already be set by the eeprom config, maybe we shouldn't override it
  // here to support weirdo nics?
  set_reg32(dev, IXGBE_AUTOC,
            (get_reg32(dev, IXGBE_AUTOC) & ~IXGBE_AUTOC_LMS_MASK) |
                IXGBE_AUTOC_LMS_10G_SERIAL);
  set_reg32(dev, IXGBE_AUTOC,
            (get_reg32(dev, IXGBE_AUTOC) & ~IXGBE_AUTOC_10G_PMA_PMD_MASK) |
                IXGBE_AUTOC_10G_XAUI);
  // negotiate link
  set_flags32(dev, IXGBE_AUTOC, IXGBE_AUTOC_AN_RESTART);
  // datasheet wants us to wait for the link here, but we can continue and wait
  // afterwards
}
...
static void start_rx_queue(struct ixy_device *dev, int queue_id) {
  ...
  struct ixgbe_rx_queue *queue =
      ((struct ixgbe_rx_queue *)(dev->rx_queues)) + queue_id;
  // 2048 as pktbuf size is strictly speaking incorrect:
  // we need a few headers (1 cacheline), so there's only 1984 bytes left for
  // the device but the 82599 can only handle sizes in increments of 1 kb; but
  // this is fine since our max packet size is the default MTU of 1518 this has
  // to be fixed if jumbo frames are to be supported mempool should be >= the
  // number of rx and tx descriptors for a forwarding application
...
static void start_tx_queue(struct ixy_device *dev, int queue_id) {
  ...
  struct ixgbe_tx_queue *queue =
      ((struct ixgbe_tx_queue *)(dev->tx_queues)) + queue_id;
  if (queue->num_entries & (queue->num_entries - 1)) {
    error("number of queue entries must be a power of 2");
  }
...
// see section 4.6.7
// it looks quite complicated in the data sheet, but it's actually really easy
// because we don't need fancy features
static void init_rx(struct ixy_device *dev) {
  // make sure that rx is disabled while re-configuring it
  // the datasheet also wants us to disable some crypto-offloading related rx
  // paths (but we don't care about them)
  ...
  for (uint16_t i = 0; i < dev->num_rx_queues; i++) {
    ...
    // enable advanced rx descriptors, we could also get away with legacy
    // descriptors, but they aren't really easier
    set_reg32(dev, IXGBE_SRRCTL(i),
              (get_reg32(dev, IXGBE_SRRCTL(i)) & ~IXGBE_SRRCTL_DESCTYPE_MASK) |
                  IXGBE_SRRCTL_DESCTYPE_ADV_ONEBUF);
    // drop_en causes the nic to drop packets if no rx descriptors are available
    // instead of buffering them a single overflowing queue can fill up the
    // whole buffer and impact operations if not setting this flag
    set_flags32(dev, IXGBE_SRRCTL(i), IXGBE_SRRCTL_DROP_EN);
    // setup descriptor ring, see section 7.1.9
    uint32_t ring_size_bytes =
        NUM_RX_QUEUE_ENTRIES * sizeof(union ixgbe_adv_rx_desc);
    ...
    struct ixgbe_rx_queue *queue =
        ((struct ixgbe_rx_queue *)(dev->rx_queues)) + i;
  ...
  // this flag probably refers to a broken feature: it's reserved and
  // initialized as '1' but it must be set to '0' there isn't even a constant in
  // ixgbe_types.h for this flag
  for (uint16_t i = 0; i < dev->num_rx_queues; i++) {
...
static void init_tx(struct ixy_device *dev) {
  ...
  for (uint16_t i = 0; i < dev->num_tx_queues; i++) {
    ...
    // setup descriptor ring, see section 7.1.9
    uint32_t ring_size_bytes =
        NUM_TX_QUEUE_ENTRIES * sizeof(union ixgbe_adv_tx_desc);
    struct dma_memory mem = memory_allocate_dma(ring_size_bytes, true);
    memset(mem.virt, -1, ring_size_bytes);
    ...
    // descriptor writeback magic values, important to get good performance and
    // low PCIe overhead see 7.2.3.4.1 and 7.2.3.5 for an explanation of these
    // values and how to find good ones we just use the defaults from DPDK here,
    ...
    // private data for the driver, 0-initialized
    struct ixgbe_tx_queue *queue =
        ((struct ixgbe_tx_queue *)(dev->tx_queues)) + i;
...
struct ixy_device *ixgbe_init(const char *pci_addr, uint16_t rx_queues,
                              uint16_t tx_queues) {
  if (getuid()) {
  ...
  struct ixy_device *dev =
      (struct ixy_device *)malloc(sizeof(struct ixy_device));
  ...
  dev->rx_queues = calloc(rx_queues, sizeof(struct ixgbe_rx_queue) +
                                         sizeof(void *) * MAX_RX_QUEUE_ENTRIES);
  dev->tx_queues = calloc(rx_queues, sizeof(struct ixgbe_tx_queue) +
                                         sizeof(void *) * MAX_TX_QUEUE_ENTRIES);
  reset_and_init(dev);
  return dev;
}
...
void ixgbe_read_stats(struct ixy_device *dev, struct device_stats *stats) {
  ...
  uint64_t rx_bytes = get_reg32(dev, IXGBE_GORCL) +
                      (((uint64_t)get_reg32(dev, IXGBE_GORCH)) << 32);
  uint64_t tx_bytes = get_reg32(dev, IXGBE_GOTCL) +
                      (((uint64_t)get_reg32(dev, IXGBE_GOTCH)) << 32);
  if (stats) {
...
// advance index with wrap-around, this line is the reason why we require a
// power of two for the queue size
...
uint32_t ixgbe_rx_batch(struct ixy_device *dev, uint16_t queue_id,
  ...
  struct ixgbe_rx_queue *queue =
      ((struct ixgbe_rx_queue *)(dev->rx_queues)) + queue_id;
  uint16_t rx_index =
      queue->rx_index; // rx index we checked in the last run of this function
  uint16_t last_rx_index = rx_index; // index of the descriptor we checked in
  ...
  for (buf_index = 0; buf_index < num_bufs; buf_index++) {
    ...
    if (status & IXGBE_RXDADV_STAT_DD) {
      if (!(status & IXGBE_RXDADV_STAT_EOP)) {
        error("multi-segment packets are not supported - increase buffer size "
              "or decrease MTU");
      }
      // got a packet, read and copy the whole descriptor
      union ixgbe_adv_rx_desc desc = *desc_ptr;
      struct pkt_buf *buf =
          (struct pkt_buf *)queue->virtual_addresses[rx_index];
      buf->size = desc.wb.upper.length;
      // this would be the place to implement RX offloading by translating the
      // device-specific flags to an independent representation in the buf
      // (similiar to how DPDK works) need a new mbuf for the descriptor
      struct pkt_buf *new_buf = pkt_buf_alloc(queue->mempool);
      if (!new_buf) {
        // we could handle empty mempools more gracefully here, but it would be
        // quite messy... make your mempools large enough
        error("failed to allocate new mbuf for rx, you are either leaking "
              "memory or your mempool is too small");
      }
      // reset the descriptor
      desc_ptr->read.pkt_addr =
          new_buf->buf_addr_phy + offsetof(struct pkt_buf, data);
      desc_ptr->read.hdr_addr = 0; // this resets the flags
      queue->virtual_addresses[rx_index] = new_buf;
      bufs[buf_index] = buf;
      // want to read the next one in the next iteration, but we still need the
      // last/current to update RDT later
  ...
  if (rx_index != last_rx_index) {
    ...
    // this is intentionally off by one, otherwise we'd set RDT=RDH if we are
    // receiving faster than packets are coming in RDT=RDH means queue is full
    set_reg32(dev, IXGBE_RDT(queue_id), last_rx_index);
    queue->rx_index = rx_index;
  }
  return buf_index; // number of packets stored in bufs; buf_index points to the
                    // next index
}

// section 1.8.1 and 7.2
// we control the tail, hardware the head
// huge performance gains possible here by sending packets in batches - writing
// to TDT for every packet is not efficient returns the number of packets
// transmitted, will not block when the queue is full
uint32_t ixgbe_tx_batch(struct ixy_device *dev, uint16_t queue_id,
                        struct pkt_buf *bufs[], uint32_t num_bufs) {
  struct ixgbe_tx_queue *queue =
      ((struct ixgbe_tx_queue *)(dev->tx_queues)) + queue_id;
  // the descriptor is explained in section 7.2.3.2.4
  // we just use a struct copy & pasted from intel, but it basically has two
  // formats (hence a union):
  // 1. the write-back format which is written by the NIC once sending it is
  // finished this is used in step 1
  // 2. the read format which is read by the NIC and written by us, this is used
  // in step 2
  ...
  // packet cleaning up must be done in batches for performance reasons, so this
  // is unfortunately somewhat complicated
  while (true) {
    // figure out how many descriptors can be cleaned up
    int32_t cleanable =
        cur_index -
        clean_index; // cur is always ahead of clean (invariant of our queue)
    ...
    // hardware sets this flag as soon as it's sent out, we can give back all
    // bufs in the batch back to the mempool
    if (status & IXGBE_ADVTXD_STAT_DD) {
      ...
      // the queue forever if you stop transmitting, but that's not a real
      // concern
      break;
    }
  }
  ...
  for (sent = 0; sent < num_bufs; sent++) {
    ...
    // always the same flags: one buffer (EOP), advanced data descriptor, CRC
    // offload, data length
    txd->read.cmd_type_len = IXGBE_ADVTXD_DCMD_EOP | IXGBE_ADVTXD_DCMD_RS |
                             IXGBE_ADVTXD_DCMD_IFCS | IXGBE_ADVTXD_DCMD_DEXT |
                             IXGBE_ADVTXD_DTYP_DATA | buf->size;
    // no fancy offloading stuff - only the total payload length
    // implement offloading flags here:
    // 	* ip checksum offloading is trivial: just set the offset
    // 	* tcp/udp checksum offloading is more annoying, you have to precalculate
